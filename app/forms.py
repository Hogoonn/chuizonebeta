...
class AcademyForm(Form):
    ...
    academy_address = StringField(
        u'주소',
        [validators.data_required(u'주소를 입력하여 위치를 찾아아주시기 바랍니다.')],
        description={'placeholder': u'주소를 입력해주세요.'}
        )
    academy_latlng = StringField(
        u'',
        [validators.data_required(u'위치를 지도위에 클릭해서, 마커를 표시해주시기 바랍니다.')],
        description={'placeholder': u'이 항목은 수동입력하지 않습니다.'}
        )